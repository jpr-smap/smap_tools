function [outref,outref_Nd]=radialmeanj(imref,varargin);
...
% imref=smap.resize_F(imref,2,'newSize');
...
%             rBins=[0:((size(imref,1)./2)+1)]./(size(imref,1)-1);
    ...
    [outref,b]=bindata((double(imref(:))),(rCoord(:)),(rBins));    
    outref(1)=cpVal;
    outref_Nd=[];

%     [a,b,test]=bindata((double(imref(:))),(rCoord(:)),(rBins));
    
    outref_Nd=reshape(b,Npix_short,Npix_long);
    
    else
    
    Npix_here=size(imref,1);
    cp_here=floor(Npix_here./2)+1;
    
    rCoord=smap.rrj(imref).*Npix_here;
    
    inds=find(rCoord<=cp_here);
    
    oddFlag=mod(size(imref,1),2);
    if( oddFlag==1 )
        % % update this to match even:
        rBins=[0:((size(imref,1)./2)+1)]./(size(imref,1)-1);
    else
%         rBins=linspace(0,0.5,(size(imref,1)./2)+1);
        rBins=linspace(0,0.5,floor((size(imref,1)./2).*sqrt(3))+1);
    end;
    rBins=rBins.*Npix_here;
    
    
    [test,y,y_new]=smap.bindata(double(imref(inds)),rCoord(inds),rBins);
    temp=zeros(size(imref));
    temp(inds)=y;
    outref=temp(cp_here,cp_here:end,cp_here);
    outref_Nd=temp;
...
% outref=smap.resize_F(outref,0.5,'newSize');
% outref_Nd=smap.resize_F(outref_Nd,0.5,'newSize');
...
%             rCoord=rCoord(:);
%             
%             [test,y,y_new]=bindata(double(gather(imref(:))),gather(rCoord),gather(rtBins),sf);
%             
%             outref_Nd=reshape(y_new,Npix,Npix);
%             
%             outref_Nd(cp(1),cp(1))=cpVal; % check this
%             
%         end;
%     end;
